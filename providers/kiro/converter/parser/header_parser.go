...
func (hp *HeaderParser) ParseHeadersWithState(data []byte, state *HeaderParseState) (map[string]HeaderValue, error) {
	...
	for offset < len(data) {
		...
		if needMoreData {
			...
			if currentPhase == initialPhase && consecutiveDataInsufficientCount > 2 {
				log.Warnf("检测到头部解析循环，强制完成解析, consecutive_count: %d, current_phase: %d, parsed_headers: %d",
					consecutiveDataInsufficientCount,
					currentPhase,
					len(state.ParsedHeaders))
			...
			if len(state.ParsedHeaders) > 0 && consecutiveDataInsufficientCount > 3 {
				log.Warnf("连续数据不足错误过多，强制完成头部解析 consecutive_count: %d, parsed_headers: %d",
					consecutiveDataInsufficientCount,
					len(state.ParsedHeaders))
				return hp.ForceCompleteHeaderParsing(state), nil
			}
			...
			if len(state.ParsedHeaders) == 0 && consecutiveDataInsufficientCount > 5 {
				log.Warnf("长期无法解析任何头部，强制使用默认头部 consecutive_count: %d",
					consecutiveDataInsufficientCount)
				return hp.ForceCompleteHeaderParsing(state), nil
			}

			log.Debugf("头部解析需要更多数据（状态已保存), parsed_headers: %d, consecutive_insufficient: %d",
				len(state.ParsedHeaders),
				consecutiveDataInsufficientCount)
	...
	if state.Phase != PhaseReadNameLength {
		log.Debugf("数据结束但解析未完成, current_phase: %d, processed_bytes: %d, parsed_headers: %d",
			state.Phase,
			offset,
			len(state.ParsedHeaders))
		...
		// 如果已经有一些解析结果，尝试强制完成
		if len(state.ParsedHeaders) > 0 {
			log.Debug("数据不完整但已有解析结果，强制完成头部解析")
			return hp.ForceCompleteHeaderParsing(state), nil
		}
...
func (hp *HeaderParser) processNamePhase(data []byte, offset *int, state *HeaderParseState) (needMoreData bool, err error) {
	...
	if availableBytes < remainingNameBytes {
		...
		*offset = len(data)
		log.Debugf("部分名称数据累积, partial_len: %d, need_len: %d",
			len(state.PartialName),
			state.NameLength)
		return true, nil
	}
...
func (hp *HeaderParser) processValueLengthPhase(data []byte, offset *int, state *HeaderParseState) (needMoreData bool, err error) {
	...
	if availableBytes < remainingBytes {
		...
		*offset = len(data)
		log.Debugf("部分值长度数据累积, partial_len: %d, need_len: %d, name: %s",
			len(state.PartialLength),
			2,
			string(state.PartialName))
		return true, nil
	}
...
func (hp *HeaderParser) processValuePhase(data []byte, offset *int, state *HeaderParseState) (needMoreData bool, err error) {
	...
	if availableBytes < remainingValueBytes {
		...
		*offset = len(data)
		log.Debugf("部分值数据累积, partial_len: %d, need_len: %d, name: %s",
			len(state.PartialValue),
			state.ValueLength,
			string(state.PartialName))
		return true, nil
	}
	...
	*offset += remainingValueBytes
	...
	if err != nil {
		log.Warnf("解析头部值失败, header_name: %s, value_type: %d, err: %v",
			string(state.PartialName),
			state.ValueType,
			err)
		// 跳过当前头部，继续处理下一个
...
func (hp *HeaderParser) parseHeaderValue(valueType ValueType, data []byte) (any, error) {
	...
	default:
		log.Warnf("未知的值类型, value_type: %d", valueType)
		// 对于未知类型，返回原始字节数据
		result := make([]byte, len(data))
		copy(result, data)
		return result, nil
