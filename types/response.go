...
// ResponseChain is a receive-only channel of *Response, used for streaming responses.
type ResponseChain <-chan *Response

// ResponseSender wraps a send-only channel and provides a context-aware Send method.
type ResponseSender chan<- *Response

// NewResponseChain creates a new buffered channel and returns a ResponseChain (read-only)
// for consumers and a ResponseSender (write-only) for producers.
func NewResponseChain(bufSize int) (ResponseChain, ResponseSender) {
	ch := make(chan *Response, bufSize)
	return ResponseChain(ch), ResponseSender(ch)
}

// Send sends a response to the channel with context-aware cancellation handling.
// It returns true on success, or false if the context is done before the send completes.
func (s ResponseSender) Send(ctx context.Context, resp *Response) bool {
	select {
	case s <- resp:
		return true
	case <-ctx.Done():
